...
let createComponent = itemList => {
  ...
  if (isValid) {
    ...
    itemList.forEach((item, index) => {
      ...
      htmlComponent +=
        "<div class='col-md-3' id='itemContainer'>\
              <button id='deletebt'class='btn btn-danger'  onclick='deleteItem(" +
        index +
        ");'><span class='glyphicon glyphicon-ban-circle'></span></button>\
              <button id='editbt' class='btn btn-success' data-toggle='modal' data-target='#myModal' onclick='rendereditItem(" +
        index +
